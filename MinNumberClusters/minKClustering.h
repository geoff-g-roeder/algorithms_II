...
public:
 UnprocessedEdge(int u_in, int v_in, int weight_in): 
   ...
   bool equals (UnprocessedEdge * other ) {
     return (get_u() == other->get_u()) && (get_v() == other->get_v());
   }
};
...
// TODO: write destructor for StaticClustering

// StaticClustering: when nodes are defined explicitly
//                   and the number of nodes is known
//                   beforehand

class StaticClustering : public Graph {
private:  
...
public:
  ...
  StaticClustering(int V_in, int k_in) {
  V = V_in;
  k = k_in;
  num_leaders = 0;
